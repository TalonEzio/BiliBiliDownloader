...
namespace BiliBiliDownloader.Wpf.Services.Implement
{
    public class SubtitleService(HttpClient httpClient, CookieContainer cookieContainer) : ISubtitleService
    {
        ...
        public async Task<SubtitleSaveInfo?> SaveSubtitleFile(string outputPath, string fileName, SubtitleResponse subtitleResponse)
        {
            try
            {
                var subtitle = subtitleResponse.SubtitleData.VideoSubtitles
                    .FirstOrDefault(x => (x.Srt.Url.Length > 0 || x.Ass.Url.Length > 0) && (x.LangKey == "vi" || x.LangKey =="en"));

                if (subtitle == null) return null;

                var url = subtitle.Srt.Url;
                ...
                var subtitleUri = new Uri(subtitle.Srt.Url);
                ...
                if (isAss)
                {
                    var assFilePath = Path.Combine(outputPath, (fileName  +'.'+ subtitle.LangKey + ".ass"));
                ...
                return await ConvertJsonToSrt(outputPath, fileName + '.' + subtitle.LangKey + ".srt", subtitleJson);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
